...
| Capacity profile assignment | `systemuser` lookup `msdyn_capacityprofileid` → `msdyn_agentcapacityprofile` (`msdyn_name`, `msdyn_totalcapacity`) | **Low** — plausible, standard-pattern guess, not confirmed | Attempted for real; degrades to `unknown` on any mismatch |
| Work-item unit cost | — | **Not attempted** | No defensible source found; always reported `unknown` in live mode. The comparison logic itself is fully implemented/tested against synthetic numbers, and demo mode exercises it end-to-end |
