...
function healthy(overrides: Partial<AgentRecord>): AgentRecord {
  return {
    ...
    presence: known({ name: "Available", isLoggedIn: true, allowsAssignment: true, capturedOn: "2026-09-21T09:02:00Z" }),
...
export const DEMO_AGENTS: AgentRecord[] = [
  ...
  // missing security role — a realistic case now that the roster is gated by role membership (see
  // config.ts's ROLE_GROUPS): a Supervisor who was never also given Agent access, not an unrelated
  // role that would no longer make it onto the roster at all.
  healthy({ id: "a-margaret", name: "Margaret Hamilton", domainName: "margaret.hamilton@contoso.com", securityRoles: known(["Customer Service Manager"]) }),
  healthy({ id: "a-hedy", name: "Hedy Lamarr", domainName: "hedy.lamarr@contoso.com", channels: known(["Chat"]) }), // voice not reachable (this product has no separate channel toggle — see checks.ts)
  healthy({
  ...
  healthy({ id: "a-annie", name: "Annie Easley", domainName: "annie.easley@contoso.com", agentCapacity: known([]) }), // no capacity profile assigned
  healthy({ id: "a-mary", name: "Mary Jackson", domainName: "mary.jackson@contoso.com", agentCapacity: known([{ profileName: "Default voice inbound", effectiveUnits: 50 }]), workItemUnitCost: known(100) }), // capacity too low
  ...
  healthy({ id: "a-elizabeth", name: "Elizabeth Feinler", domainName: "elizabeth.feinler@contoso.com", presence: known({ name: "Away", isLoggedIn: true, allowsAssignment: false, capturedOn: "2026-09-21T09:40:00Z" }) }), // presence warn: away
  ...
  healthy({ id: "a-feifei", name: "Fei-Fei Li", domainName: "feifei.li@contoso.com", agentCapacity: unknownField("Could not read capacity profile assignment (demo).") }), // capacity unknown
  healthy({
    id: "a-mae", name: "Mae Jemison", domainName: "mae.jemison@contoso.com",
    agentCapacity: known([{ profileName: "Default voice inbound", effectiveUnits: null }, { profileName: "Default voice outbound", effectiveUnits: null }])
  }), // real-world case: profile(s) assigned but no value set anywhere (join row or profile default)
  healthy({
    id: "a-joy", name: "Joy Buolamwini", domainName: "joy.buolamwini@contoso.com",
    agentCapacity: known([{ profileName: "Default voice inbound", effectiveUnits: 1 }]),
    workItemUnitCost: known(null),
    hasProfileBasedReachableWorkstream: known(true)
  }), // real-world case: reachable workstream uses "Profile based" capacity — capacity 1 is genuinely sufficient, no unit comparison applies
  healthy({ id: "a-shafi", name: "Shafi Goldwasser", domainName: "shafi.goldwasser@contoso.com", capacityBlocked: known(true) }), // currently blocked from new work — at capacity across their profile(s)
  healthy({
  ...
  healthy({
    ...
    // channels and workItemUnitCost are both derived from the same routing-configuration read, so
    // a permission gap on that one table realistically takes both out together, not independently.
    channels: unknownField("Could not read the routing configuration needed to determine channel access: no permission to read \"msdyn_routingconfigurationstep\" (demo)."),
    ...
    // presence and capacityBlocked both come from the same msdyn_agentstatus row in real life, so a
    // permission gap on that table realistically takes both out together, not independently.
    presence: unknownField("Could not read this agent's current presence (demo)."),
    capacityBlocked: unknownField("Could not read this agent's current status (demo).")
  }), // the realistic "everything else is fine, but the genuinely-unverifiable fields are unknown" common case
  healthy({
  ...
  // hiding (unlike a live connection, demo mode isn't pre-filtered by dataverse.ts).
  healthy({ id: "a-service", name: "Integration Service Account", domainName: "svc-integration@fabrikam-systems.com", securityRoles: known(["Sales Manager"]) })
];
